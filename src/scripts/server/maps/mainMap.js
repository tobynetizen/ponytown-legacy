...
const mainMapData = JSON.parse(fs.readFileSync(paths_1.pathTo('data', 'maps', 'main.json'), 'utf8'));
...
function createCookieTable2(x, y) {
    return entities.cookieTable2(x, y + 0.5);
}
...
function addSeasonalObjects(world, map, season, holiday) {
    ...
    function addHolly(x, y) {
        add(entities.holly(x, y + (1 / constants_1.tileHeight)));
    }
    function addHollyDecoration(x, y, a = true, b = true, c = true, d = true) {
        if (isChristmas) {
            a && addHolly(x - 2.8, y);
            b && addHolly(x - 1, y);
            c && addHolly(x + 1, y);
    ...
    function addGraveWithGhost(x, y, tombType) {
...
function createMainMap(world) {
    ...
    function addTree(x, y, variant, web = false, spider = false) {
    ...
    function cliffRightWithTrim(x, y, h) {
        cliffRight(x, y - 3, h - 3);
        cliffTrimRight(x + 1, y, h);
    }
    function cliffLeftWithTrim(x, y, h) {
        cliffLeft(x, y - 3, h - 3);
        cliffTrimLeft(x, y, h);
    }
    ...
    function cliffTrimRight(x, y, h, botTrim = true) {
        if (botTrim) {
    ...
    function cliffSESection(x, y, w) {
        for (let i = 0; i < w; i++) {
            cliffSE(x + i, y - i);
        }
    }
    function cliffSWSection(x, y, w) {
        for (let i = 0; i < w; i++) {
            cliffSW(x + i, y + i);
        }
    }
    ...
    const plankOffsets = [0, -1, 0, -2, -1, -1, 0, -2, -1].map(x => x / constants_1.tileWidth);
    for (let y = 0; y < 9; y++) {
        const maxX = y < 5 ? ((y % 2) ? 5 : 4) : ((y % 2) ? 3 : 2);
        const baseX = pierX + 4.5 + ((y % 2) ? 0 : (plankWidth / 2)) + plankOffsets[y];
        const baseY = pierY + 71 - (9 / constants_1.tileHeight);
        for (let x = 0; x < maxX; x++) {
            if ((x === 0 && (y % 2)) || (x === (maxX - 1) && (y % 2))) {
                const ox = x === 0 ? (18 / constants_1.tileWidth) : (-18 / constants_1.tileWidth);
