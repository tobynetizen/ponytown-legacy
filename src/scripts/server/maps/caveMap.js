...
const mapData = JSON.parse(fs.readFileSync(paths_1.pathTo('data', 'maps', 'cave.json'), 'utf8'));
function createCaveMap(world) {
    ...
    function caveSStart(x, y) {
        add(entities.caveS1(x + 0.5, y - 1));
        cracksS(x, y);
    }
    function caveSEnd(x, y) {
    ...
    function caveRightWithTrimNoEdge(x, y, h) {
        caveRight(x, y, h);
        caveTrimRight(x + 1, y, h, false);
    }
    function caveLeftWithTrimNoEdge(x, y, h) {
        caveLeft(x, y, h);
        caveTrimLeft(x, y, h, false);
    }
    function caveRightWithTrim(x, y, h) {
        caveRight(x, y - 3, h - 3);
        caveTrimRight(x + 1, y, h);
    }
    function caveLeftWithTrim(x, y, h) {
        caveLeft(x, y - 3, h - 3);
        caveTrimLeft(x, y, h);
    }
    function caveLeft(x, y, h) {
        for (let i = 0; i < h; i++) {
            add(entities.caveTopW(x + 0.5, y - i));
        }
    }
    function caveRight(x, y, h) {
        for (let i = 0; i < h; i++) {
            add(entities.caveTopE(x + 0.5, y - i));
        }
    }
    function caveTrimLeft(x, y, h, botTrim = true) {
        if (botTrim) {
    ...
    function caveTrimRight(x, y, h, botTrim = true) {
        if (botTrim) {
    ...
    function caveSESection(x, y, w) {
        for (let i = 0; i < w; i++) {
            caveSE(x + i, y - i);
        }
    }
    function caveSWSection(x, y, w) {
        for (let i = 0; i < w; i++) {
            caveSW(x + i, y + i);
        }
    }
    ...
    function railsH(x, y, length) {
        for (let i = 0; i < length; i++) {
            add(entities.mineRailsH(x + i + 0.5, y));
        }
    }
    function railsV(x, y, length) {
        for (let i = 0; i < length; i++) {
    ...
    map.controllers.push(new controllers_1.FlyingCritterController(world, map, entities.bat, 2, 10, () => true, true));
    const wallController = new wallController_1.WallController(world, map, entities.stoneWalls);
