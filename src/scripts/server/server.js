...
if (config_1.args.tools) {
    const toolsPage = index.user(production, '/tools/', 'style-tools.css', 'bootstrap-tools.js', undefined, true, !!config_1.args.local, false);
...
if (config_1.args.login) {
    ...
    const userPage = index.user(production, '/', 'style.css', 'bootstrap.js', socketOptions, false, !!config_1.args.local, !production);
    ...
    const csp = `object-src 'none';`
        ...
        + `script-src 'unsafe-eval' ${script} ${analytics};`;
    const linkPreloads = [
        ...userPage.preload,
    ];
