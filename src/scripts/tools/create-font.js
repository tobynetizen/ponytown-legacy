...
function createFont(canvas, w, h, addImage, options = {}) {
    ...
    const baseCodes = charsToCodes(exports.BASE_CHARS);
    const charsCodes = charsToCodes(exports.CHARS);
    const chineseCodes = options.noChinese ? [] : charsToCodes(exports.CHINESE);
    const romajiCodes = charsToCodes(exports.ROMAJI);
...
function isColEmpty(data, x, y, h) {
...
function getXY(cols, i) {
    return { x: i % cols, y: Math.floor(i / cols) };
}
function getCharWidth(data, x, y, w, h) {
    let left = 0;
    let width = w;
    for (; left < w; left++) {
...
function lengthChecker(expected) {
