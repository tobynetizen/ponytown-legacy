...
const gamepadMappings_1 = require("./gamepadMappings");
...
function detectMapping(id, browser) {
    for (let i = 0; i < gamepadMappings_1.GAMEPAD_MAPPINGS.length; i++) {
        if (isCompatible(gamepadMappings_1.GAMEPAD_MAPPINGS[i], id, browser)) {
            return gamepadMappings_1.GAMEPAD_MAPPINGS[i];
        }
    }
    return gamepadMappings_1.GAMEPAD_MAPPINGS[0];
}
