...
if (args.tools) {
	const toolsPage = index.user(
		production, '/tools/', 'style-tools.css', 'bootstrap-tools.js', undefined, true, !!args.local, false);
...
if (args.login) {
	...
	const userPage = index.user(
		production, '/', 'style.css', 'bootstrap.js', socketOptions, false, !!args.local, !production);
